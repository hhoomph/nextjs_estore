...
class CartStorage {
  private static generateGuestId(): string {
    return `${CART_CONSTANTS.GUEST_ID_PREFIX}${randomUUID()}`;
  }
  ...
  static saveCartState(state: CartState): void {
    ...
    try {
      ...
      // Ensure all items have Date objects for addedAt and updatedAt
      ...
      return {
        ...
        pendingUpdates: parsed.pendingUpdates.map((update) => ({
          ...
          item: {
            ...
            addedAt: new Date(update.item.addedAt ?? new Date()),
            updatedAt: new Date(update.item.updatedAt ?? new Date()),
          } as EnhancedCartItem,
        ...
        (set, get) => ({
          ...
          syncWithServer: async () => {
            ...
            try {
              ...
              // Send pending updates to server with retry
          ...
          getItemCount: () => {
            ...
            return Math.max(
              0,
              safeItems.reduce((count, item) => count + item.quantity, 0),
            );
          },
            ...
            getItem: (name) => {
              ...
              if (typeof window === "undefined") {
                return {
                  state: {
                    items: [],
                    guestId: CartStorage.getGuestId(),
                    isGuest: true,
                    lastSync: new Date(),
                    pendingUpdates: [],
                    isLoading: false,
...
// Export utilities
export { CartStorage, CartValidator };
